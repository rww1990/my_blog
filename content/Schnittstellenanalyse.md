...
flowchart TD
    A[Schnittstelle] --> B[Was wird übertragen?]
    A --> C[Wann erfolgt der Austausch?]
    A --> D[Wie ist sie gestaltet?]

    B --> B1[Daten / Befehle]
    C --> C1[Regelmäßig / Ereignisgesteuert]
    D --> D1[Technisch: API, Datei]
    D --> D2[Organisatorisch: Prozess, Meeting]
